...
<table class="table table-bordered table-striped {{ count($ledger_categories) > 0 ? 'datatable' : '' }}">
    ...
    <tbody>
        @if (count($ledger_categories) > 0)
            @foreach ($ledger_categories as $expense_category)
                <tr data-entry-id="{{ $expense_category->id }}">
                    <td field-key='name'>{{ $expense_category->name }}</td>
                                <td field-key='created_by'>{{ $expense_category->created_by->name or '' }}</td>
                                                                <td>
                                    @can('view')
                                    <a href="{{ route('ledger_categories.show',[$expense_category->id]) }}" class="btn btn-xs btn-primary">@lang('quickadmin.qa_view')</a>
                                    @endcan
                                    @can('edit')
                                    <a href="{{ route('ledger_categories.edit',[$expense_category->id]) }}" class="btn btn-xs btn-info">@lang('quickadmin.qa_edit')</a>
                                    @endcan
                                    @can('delete')
{!! Form::open(array(
                                        ...
                                        'route' => ['ledger_categories.destroy', $expense_category->id])) !!}
